...
Act* d2common_load_act_run(D2Version gameVersion, int actId, int seed, int difficulty) {
    if (gameVersion == VersionDiablo2) return D2COMMON_Pod_LoadAct(actId, seed, TRUE, FALSE, difficulty, (DWORD)NULL, 1, D2CLIENT_Pod_LoadAct_1, D2CLIENT_Pod_LoadAct_2);
    return NULL;
}
...
void d2common_remove_room_data(D2Version gameVersion, Act* pAct, Level* pLevel, Room2* pRoom2) {
    if (gameVersion == VersionDiablo2) return D2COMMON_Pod_RemoveRoomData(pAct, pLevel->dwLevelNo, pRoom2->dwPosX, pRoom2->dwPosY, NULL);
}
